...
@app.callback(
    ...
    Output('final-triangulate-data', 'data'),
    ...
    State('final-triangles-data', 'data'),
...
def update_graph(contents, submit_points_clicks, triangulate_clicks,final_triangles_clicks, color_clicks, camera_clicks, filename, manual_input, polygon, triangles, coloring, camera_data):
    ...
    if button_id == 'upload-polygon' and contents:
        ...
        return False, polygon, dash.no_update,dash.no_update, dash.no_update, dash.no_update, fig, dash.no_update
    ...
    if button_id == 'final-triangulate-button' and final_triangles_clicks > 0 and polygon:
        triangles = triangulation.ear_clipping_triangulation(polygon)
        fig = plotting.animate_triangulation(polygon)
        return False, polygon, triangles, triangles,dash.no_update, dash.no_update, fig, dash.no_update
    ...
    if button_id == 'color-button' and color_clicks > 0 and polygon:
        ...
        return False, polygon, triangles,triangles, coloring_serializable, dash.no_update, fig, dash.no_update
    ...
    if button_id == 'camera-button' and camera_clicks > 0 and polygon:
        ...
        return False, polygon, triangles,triangles, coloring, camera_positions, fig, camera_message
...
def update_graph(contents, submit_points_clicks, triangulate_clicks, color_clicks, camera_clicks, filename, manual_input, polygon, triangles, coloring, camera_data):
    ...
    if button_id == 'triangulate-button' and triangulate_clicks > 0 and polygon:
        triangles = triangulation.ear_clipping_triangulation(polygon)
        fig = plotting.plot_polygon(polygon, triangles)
...
def animate_colorization(triangles,polygon):
    ...
    frames.append(go.Frame(data=frame_data, name='All Triangles'))
...
def plot_coloring(polygon,triangles):
    ...
    for i in range(len(index_to_triangle)):
        print(index_to_triangle[i])
        x_tri, y_tri = zip(*index_to_triangle[i])
